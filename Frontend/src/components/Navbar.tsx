...
const Navbar = () => {
  ...
  const getLinkClass = (path: string) => {
    return location.pathname === path
      ? "text-accent transition duration-200"
      : "text-white hover:text-blue-300 transition duration-200";
  };

  const getMobileLinkClass = (path: string) => {
    return location.pathname === path
      ? "block text-accent transition"
      : "block text-white hover:text-blue-300 transition";
  };
  ...
  return (
    <>
      ...
      <nav className="bg-navbar text-white sticky top-0 z-50 shadow-lg">
        <div className="w-full px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6 md:gap-20">
            ...
            <div className="flex items-center gap-4">
              <Link
                to="/cart"
                className="relative hover:text-blue-300 transition text-accent"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-6 w-6"
          ...
          <div className="md:hidden bg-blue-800 px-6 py-4 flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3 text-sm font-medium">
              ...
              <li>
                <Link
                  to="/cart"
                  className={getMobileLinkClass("/cart")}
                  onClick={() => setIsOpen(false)}
                >
                  🛒 Cart
                </Link>
            ...
            <button className="text-left text-sm hover:text-blue-300 transition text-white">
              👤 Profile
            </button>
          </div>
        )}
